...
class Tokenizer:
    ...
    def from_directory(cls, tokenizer_dir=None, dataset=None):
        ...
        with open(os.path.join(resolved_dir, "tokenizer.pkl"), "rb") as f:
            # tokenizer.pkl is a trusted local artifact written by this script's
            # own tokenizer-training step — not a downloaded/untrusted file.
            enc = pickle.load(f)
...
def safe_torch_load(*args, **kwargs):
    ...
    code execution. We fall back to weights_only=False only when a checkpoint
    carries non-tensor objects the restricted loader rejects. These are
    trusted local artifacts generated by this script, so the fallback keeps
    every existing load working unchanged.

    The sole call site (get_token_bytes) passes an open file handle rather
    than a path. A failed weights_only=True attempt partially consumes that
    handle, so the fallback must rewind it first or it reads from the wrong
    offset and raises an unrelated UnpicklingError instead of loading.
    """
...
def get_token_bytes(device="cpu", dataset=None):
    ...
    with open(path, "rb") as f:
        # token_bytes.pt is a trusted local artifact written by this script.
        return safe_torch_load(f, map_location=device)
